...
static int check_status_global = 0;
static int test_counter = 0; //just for test,we can delete it;
void *check_thread();
pthread_mutex_t check_status_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void * check_thread()
{
	...
	while(1)
	{
		check_status_global = (check_status_global+1) % 2;
		sleep(2);
	}
}
#endif
...
/*这段代码仅仅是测试使用*/
/*故障处理算法需要在该函数内实现*/
/*Input: 一帧数据 frame_pointer*/
/*Output: 检测状态信息:Normal_status(0) or Error_status(1)*/
/**************************************/
int do_with_frame(char *frame)
{
	/*just for test*/
	int test_check_status = 0;
	...
	if(test_counter == 65535)
		test_counter = 0;
	if( test_counter % 2 )
		test_check_status = Normal_status;
	else
		test_check_status = Error_status;
...
void *check_thread()
{
    ...
    char buffer[BUFFER_SIZE] = {0};
    struct timeval timestamp;
	int check_status = Normal_status;
    DBG("Enter check_thread\n");

    /*If we have more than one input_plugin ,we should change here */
	/*But now we just use one input_plugin,to keep things simple, i make it be a constant*/
	/*Base this number ,we can know which one input_plugin's frame we used*/
	int input_number = 0;
    ...
    while(!pglobal->stop) {
		...
		/*Now，we get a framer ,jsut do with it; */
		/*call the check_error_alogrithm function*/
		
		check_status = do_with_frame(frame);

		pthread_mutex_lock(&check_status_mutex);
			check_status_global = check_status;//Normal_status(0) or Error_status(1)
...
void *client_thread(void *arg)
{
    ...
    if(strstr(buffer, "GET /?action=snapshot") != NULL) {
        ...
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /check_error.json") != NULL) {
        req.type = A_CHECK_ERROR_JSON;
        input_suffixed = 255;
    ...
    case A_PROGRAM_JSON:
        ...
        break;
	case A_CHECK_ERROR_JSON:
        DBG("Request for the check_error  JSON file\n");
        send_Check_Error_JSON(lcfd.fd);
        break;
...
void *server_thread(void *arg)
{
    ...
    /*creat a child for checking error pic*/
	pthread_create(&checker,NULL,check_thread,NULL);
...
/******************************************************************************
Description.: Send a JSON file which is contains information about the check_message
Input Value.: fildescriptor fd to send the answer to
Return Value: -
******************************************************************************/

#define CHECK_MESSAGE_JSON     "{\n" \
									"\"check_message\":[\n" \
									"{\n" \
										"\"status\":\"%s\"\n" \
									"}\n" \
...
void send_Check_Error_JSON(int fd)
{
    char buffer[BUFFER_SIZE*2] = {0}; // FIXME do rea llocation if the buffer size is small 
	int check_status = Normal_status;
